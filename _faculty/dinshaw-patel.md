---
layout: faculty-profile
title: "Dinshaw Patel, PhD"
name: "Dinshaw Patel"
degree: "PhD"
position: "Member; Abby Rockefeller Mauzé Chair in Experimental Therapeutics"
institution: "MSK"
lab_name: "Patel Laboratory"
email: "pateld@mskcc.org"
accepting_students: false
sort_key: "patel dinshaw"
description: "Structural biology of nucleic acids and nucleic acid-binding proteins, including CRISPR-Cas complexes and epigenetic regulation."
description_short: "Structural biology of nucleic acids and nucleic acid-binding proteins, including CRISPR-Cas complexes and epigenetic regulation."
education:
  - degree: "PhD"
    year: 1968
    institution: "New York University"
  - degree: "MS"
    year: 1963
    institution: "California Institute of Technology"
  - degree: "BSc"
    year: 1961
    institution: "University of Bombay"
# Education. edu_doc/edu_ms/edu_undergrad come from
# tpcb_faculty_cleaned.csv; edu_phd is retained from tpcb_faculty.csv
# for provenance and is NOT rendered (the two files disagree - see notes).
edu_doc: "New York University"
edu_ms: "California Institute of Technology"
edu_undergrad: "University of Bombay"
edu_phd: "Columbia University"
research_approach:
  - "Structural Biology"
research_focus:
  - "Gene Expression & RNA"
  - "Epigenetics & Chromatin"
notable_honors:
  - "Member, National Academy of Sciences"
  - "Member, American Academy of Arts and Sciences"
lab_website: "https://www.mskcc.org/research/ski/labs/dinshaw-patel"
tpcb_profile_url: "https://chembio.triiprograms.org/faculty-research/faculty-directory/dinshaw-patel-phd/"
profile:
  image: logos/headshot-placeholder.png
  alt: "Photo of Dinshaw Patel"
---

The Patel laboratory is internationally recognized for structural and biophysical studies of nucleic acid structures, chemical modifications, and their interactions with proteins. The lab uses X-ray crystallography, NMR spectroscopy, and cryo-electron microscopy to determine atomic structures of DNA and RNA in biologically important conformations—including G-quadruplexes, i-motifs, DNA mismatches, and unusual helical structures—as well as complexes of nucleic acids with small molecules and proteins relevant to transcription, epigenetics, and DNA repair. A notable research direction is the structural characterization of CRISPR-Cas effector complexes, providing atomic details of target recognition and cleavage that inform applications in genome editing and anti-phage defense.
