---
layout: faculty-profile
title: "Luciano Marraffini, PhD"
name: "Luciano Marraffini"
degree: "PhD"
position: "Professor"
institution: "Rockefeller"
lab_name: "Laboratory of Bacteriology"
email: "marraffini@rockefeller.edu"
accepting_students: true
sort_key: "marraffini luciano"
description: "Mechanisms of CRISPR-Cas adaptive immunity in bacteria and defense against horizontal gene transfer."
description_short: "Mechanisms of CRISPR-Cas adaptive immunity in bacteria and defense against horizontal gene transfer."
# Education. edu_doc/edu_ms/edu_undergrad come from
# tpcb_faculty_cleaned.csv; edu_phd is retained from tpcb_faculty.csv
# for provenance and is NOT rendered (the two files disagree - see notes).
edu_doc: "University of Chicago"
edu_undergrad: "Universidad Nacional de Rosario"
edu_phd: "Universidad Nacional de Rosario"
research_approach:
  - "Chemical Cell Biology"
research_focus:
  - "Infectious Disease"
  - "Gene Expression & RNA"
notable_honors:
  - "HHMI Investigator"
  - "Member NAS"
  - "Member American Academy of Arts and Sciences"
  - "NIH Director's Pioneer Award"
  - "NIH Director's New Innovator Award"
  - "Searle Scholar"
  - "Hans Sigrist Prize"
  - "Vilcek Prize in Biomedical Science"
lab_website: "https://www.rockefeller.edu/our-scientists/heads-of-laboratories/982-luciano-marraffini/"
personal_lab_website: "https://marraffini.rockefeller.edu"
tpcb_profile_url: "https://chembio.triiprograms.org/faculty-research/faculty-directory/luciano-marraffini-phd/"
profile:
  image: logos/headshot-placeholder.png
  alt: "Photo of Luciano Marraffini"
---

The Marraffini laboratory studies the biology of CRISPR-Cas systems in bacteria, with a particular focus on understanding how these immune systems defend against bacteriophage infection and horizontal gene transfer. Professor Marraffini's group made foundational contributions to establishing that CRISPR-Cas systems can target DNA—a discovery critical to the development of CRISPR-based genome editing technologies. Current research investigates the molecular mechanisms by which diverse CRISPR-Cas systems acquire new spacers from invading nucleic acids (adaptation), recognize and destroy foreign DNA or RNA (interference), and are regulated to avoid autoimmunity. The lab also explores the evolutionary arms race between bacteria and phage, including phage-encoded anti-CRISPR proteins, to understand how immunity evolves in natural microbial communities.
